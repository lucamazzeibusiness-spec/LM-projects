...
function FilterChip({ label, active, onPress, variant }: { label: string; active: boolean; onPress: () => void; variant: 'red' | 'navy' }) {
  const activeClass = variant === 'red' ? 'border-db-red bg-db-red' : 'border-db-navy bg-db-navy dark:border-[#3A4453] dark:bg-[#3A4453]'
  return (
    <Pressable
      onPress={onPress}
      className={`rounded-full border px-3 py-1.5 ${active ? activeClass : 'border-db-gray-200 dark:border-[#2A323D] bg-white dark:bg-[#171C24]'}`}
    >
      <Text className={`text-xs font-medium ${active ? 'text-white' : 'text-db-navy-light dark:text-[#9AA4B0]'}`}>{label}</Text>
    </Pressable>
  )
}
...
export default function Aufgaben() {
  ...
  return (
    <ScrollView className="flex-1 bg-db-gray-50 dark:bg-[#10141B]" contentContainerClassName="gap-4 p-4">
      <View>
        <Text className="text-xl font-semibold text-db-navy dark:text-[#EEF1F4]">Aufgaben</Text>
        <Text className="text-sm text-db-navy-light dark:text-[#9AA4B0]">Von deinem Ausbilder zugewiesen, mit Lernziel</Text>
      </View>
      ...
      <View className="gap-3">
        {gefiltert.map((a) => (
          <Pressable
            ...
            className="rounded-xl border border-db-gray-200 dark:border-[#2A323D] bg-white dark:bg-[#171C24] p-4"
          >
            <View className="flex-row items-start justify-between gap-3">
              <View>
                <Text className="text-xs font-medium text-db-navy-light dark:text-[#9AA4B0]">{a.id}</Text>
                <Text className="text-sm font-semibold text-db-navy dark:text-[#EEF1F4]">{a.titel}</Text>
              </View>
            ...
            <View className="mt-2 flex-row items-start gap-1.5">
              ...
              <Text className="flex-1 text-xs text-db-navy dark:text-[#EEF1F4]">{a.lernziel}</Text>
            </View>
            <View className="mt-3 flex-row flex-wrap items-center gap-2">
              <GewerkBadge gewerk={a.gewerk} />
              <PrioBadge prioritaet={a.prioritaet} />
              <Text className="text-xs text-db-navy-light dark:text-[#9AA4B0]">Fällig: {a.faelligkeit}</Text>
            </View>
          </Pressable>
        ))}
        {gefiltert.length === 0 && (
